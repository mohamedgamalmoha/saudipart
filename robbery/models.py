...
NationalIDValidator = RegexValidator(r'^[0-9]{10}', 'مطلوب عشر ارقام صجيجة قثط')
PhoneNumberValidator = RegexValidator(r'^[0-9]{10}$', 'مطلوب عشر ارقام صجيجة قثط')
IbanValidator = RegexValidator(r'^[a-zA-Z0-9]{24}$', 'مطلوب اربعة وعشرون احرف وارقام فقط (A-Z & 0-9)')
...
class Robbery(models.Model):
    ...
    mediator_iban = models.CharField('اي بان الوسيط', blank=True, null=True, max_length=150, validators=[IbanValidator])
    gang_iban = models.CharField('اي بان العصابة', max_length=150, validators=[IbanValidator])
